...
- Calling field accessor methods (`GetString`, `GetInt32()`, etc) now throw an exception when called when the reader is in an invalid state.
  ...
  as previously the behavior was not specified, but might return the first or last row of data. #159
- Fixes an issue that prevented some .xls files from being read correctly. #139
